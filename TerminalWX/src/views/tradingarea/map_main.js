/**********************
    地图导航方法
 **********************/
import { line_points } from '@/components/mapNavigation/map_data.js';
...
export let map_config = {
	...
	twoPointsTime: 10,	// 两个箭头间的距离  单位:m
...
export let Map_fun = {
	...
	changeMapStyle(to, callback) {
		let mapCenterX = map_config.iCol / 2 - 32, //地图容器的中心坐标 
			mapCenterY = map_config.iRow / 2 + 3,
		...
		//修改v_touch
		this.setTouchData(diffX, diffY);
	},

	/**
	 * 修改v_touch中的scale, left top
	 */
	setTouchData(left, top) {
		v_touch.scaleVal = this.scale;
		v_touch.left = left;
		v_touch.top = top;
		v_touch.rotateVal = 0;
	},
	...
	findBestPath(to) {
		...
		if(pointArr.length > 0) {
			...
			this.distance = Math.floor((this.bestPathArr.length - 1) * map_config.twoPointsTime);
			log(`需要${ this.distance }m`);
		} else {
			console.log('无最佳路径');
			this.bestPathArr = '';
		}
	 ...
	 */
	clearMap() {
		Map_fun.mapLineCon.empty();
		clearInterval(Map_fun.addArrowTimer);
	},
	 ...
	 */
	parseInt(a) {
		a.x = parseInt(a.business_position_x);
		a.y = parseInt(a.business_position_y);
		return a;
	}
};	
...
let facilitie_fun = {
	...
	show(x, y) {
		...
		Map_fun.findBestPath({x, y});
	},

	delete(e) {
		Map_fun.facObj.show();
		Map_fun.business_id = $(e).data('id');
	},
};
