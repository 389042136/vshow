var Vshow = {
    ...
    mouseEffect: function (e, t, a) {
        ...
        function r() {
            for (var i = 0; i < s.length; i++) s[i].alpha <= 0 ? (t.body.removeChild(s[i].el), s.splice(i, 1)) : (s[i].y--, s[i].scale += .004, s[i].alpha -= .013, s[i].el.style.cssText = "left:" + s[i].x + "px;top:" + s[i].y + "px;opacity:" + s[i].alpha + ";transform:scale(" + s[i].scale + "," + s[i].scale + ") rotate(45deg);background:" + s[i].color + ";z-index:99999");
					...
					name1.css("bottom","+="+ shift*2+"px").show().animate({"bottom":"-="+shift+"px"},interval,function(){
						name2.css("bottom","+="+shift*2+"px").show().animate({"bottom":"-="+shift+"px"},interval,function(){
							name3.css("bottom","+="+shift*2+"px").show().animate({"bottom":"-="+shift+"px"},interval,function(){
								setTimeout(function(){Vshow.cicleFun(name1,name2,name3,shift,interval)},1500);
							});
						});
					});
				});
    ...
    playRain: function () {
        ...
        while (increment < 100) {
            ...
            drops += '<div class="rain-drop" style="left: ' + increment + '%; bottom: ' + (randoFiver + randoFiver - 1 + 80) + '%; animation-delay: 0.' + randoHundo + 's; animation-duration: 0.5' + randoHundo + 's;"><div class="rain-stem" style="animation-delay: 0.' + randoHundo + 's; animation-duration: 0.5' + randoHundo + 's;"></div><div class="rain-splat" style="animation-delay: 0.' + randoHundo + 's; animation-duration: 0.5' + randoHundo + 's;"></div></div>';
            backDrops += '<div class="rain-drop" style="right: ' + increment + '%; bottom: ' + (randoFiver + randoFiver - 1 + 80) + '%; animation-delay: 0.' + randoHundo + 's; animation-duration: 0.5' + randoHundo + 's;"><div class="rain-stem" style="animation-delay: 0.' + randoHundo + 's; animation-duration: 0.5' + randoHundo + 's;"></div><div class="rain-splat" style="animation-delay: 0.' + randoHundo + 's; animation-duration: 0.5' + randoHundo + 's;"></div></div>';
        }
