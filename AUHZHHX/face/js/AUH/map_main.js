...
var Map_fun = {
	...
	calcScale: function(nowPos, gate) {
		var nowPos = Map_fun.parseInt(nowPos), destPos;
		//渲染当前坐标
	...
	crossFloor(nowPos, destPos) {
		var arr = [];
		if(nowPos.floor == 'F4') {
			/*arr = Map_fun.findBestPath(nowPos, F3_F4_connection.F4);
			if(!arr) {
				console.log('4层中断');
				return;
			};*/
...
/*var facilitiesObj = $('.airport-facilities');
$('.map-spot-con, .map-line').click(function(e){
	facilitiesObj.find('[sign]').css({
		width: '0.4rem',
		height: '0.5rem'
	})
	
	var R = 5 / Map_fun.scale > 1 ? 5 / Map_fun.scale : 1;
	var posX = Math.round((e.pageX - $(this).offset().left) / (mapWidth * Map_fun.scale) * map_config.iCol);
	var posY = Math.round((e.pageY - $(this).offset().top) / (mapHeight * Map_fun.scale) * map_config.iRow);
	for(var i in facilitiesData){
		for(var j in facilitiesData[i]){
			if(Math.abs(posX - facilitiesData[i][j].x) <= R && Math.abs(posY - facilitiesData[i][j].y) <= R){
				var obj = `[sign="${i}-${facilitiesData[i][j].x}-${facilitiesData[i][j].y}"]`;
				$(obj).velocity({
					width: '0.5rem',
					height: '0.6rem'
				}, 100);
	...
	Map_fun.scaleMap(Map_fun.parseInt(initObject.devicePos), 0, 5);
})*/